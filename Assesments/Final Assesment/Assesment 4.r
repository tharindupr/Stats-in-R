library(readxl)
AS <-  read_excel("C:/Stats in R/Final Assesment/Assessment 04 Dataset 2020.xlsx")
View(AS)
...
library(tidyr)
library(ggplot2)
(gs<-AS[-1] %>% gather(-`Body fat (%)`, key=var, value = "value") %>%
    ggplot(aes(x=value, y = `Body fat (%)`))+ geom_point()+
    facet_wrap(~ var, scales = "free")+theme_bw())
gs+labs(x="Predictors", y = "Body fat (%)")+
...
names(AS)<-c("CaseNum", "BodyFat", "Age","Chest","Density","Knee","Weight")
...
(cm<-corr.test(AS[,3:7])$r) #focuses on predictors only #runoff the correlation on explanatory variables
...
fit1<-lm(BodyFat~Age+Chest+Density+Knee+Weight,AS) #lm(Sales~. , data = AS[,-1])
summary(fit1)
...
#remove Wight
fit2<-lm(BodyFat~Age+Chest+Density+Knee,AS) #lm(Sales ~. , data = AS[,c(-1,-3)])
summary(fit2)
(v2<-vif(fit2))

#remove insignificant contributor
fit3<-lm(BodyFat~Age+Chest+Density,AS) #lm(Sales ~. , data = AS[,c(-1,-3,-7)])
summary(fit3)
(v3<-vif(fit3))

###summarising steps to selecting model
Names<-c("Intercept","Age", "Chest Circumference", "Density",
...
### re-order variables 
allmodel<-(Frame<-Frame[,c(1:4,7,5:6)])
...
Labels<-c("Overall", "Intercept", "Age", "Chest Circumference", "Density")
(output<-sumary(fit3))
########################Overall
##################adjusted r squared
...
(final_mn<-mn+labs(x="Residuals", y="Frequency")+
